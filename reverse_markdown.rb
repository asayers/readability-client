...
module ReverseMarkdown
  class Mapper
    ...
    def process_root(element)
      ...
      self.links_array.each_with_index do |link, reference|
        markdown += "\n[#{reference+1}]: #{link}"
      end
    ...
    def ending(element)
      ...
      case element.name.to_sym
        ...
        when :a
          if !element.text.strip.empty? && element['href'] && !element['href'].start_with?('#')
            self.links_array << element['href'] + title_markdown(element)
            "][#{self.links_array.length}]"
          else
            ""
          end
        when :img
          self.links_array << element['src'] + title_markdown(element)
          "#{element['alt']}][#{self.links_array.length}] "
        else
